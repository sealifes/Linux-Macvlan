				 ...
				 const struct ethhdr *eth, bool local)
{
	...
	if (skb_vlan_tag_present(skb)) {
		...
		skb_vlan_push(skb, skb->vlan_proto, skb->vlan_tci);
		eth = eth_hdr(skb);
	}
...
static int macvlan_broadcast_one_xmit(struct sk_buff *skb,
				      const struct macvlan_dev *vlan,
				      const struct ethhdr *eth, bool local)
{
	struct net_device *dev = vlan->dev;
...
static void macvlan_broadcast_xmit(struct sk_buff *skb,
				   const struct macvlan_port *port,
				   struct net_device *src,
				   enum macvlan_mode mode)
{
	...
	hash_for_each_rcu(port->vlan_hash, i, vlan, hlist) {
		...
		nskb = skb_clone(skb, GFP_ATOMIC);
		if (likely(nskb))
			err = macvlan_broadcast_one_xmit(nskb, vlan, eth,
							mode == MACVLAN_MODE_BRIDGE) ?:
				  netif_rx(nskb);
		macvlan_count_rx(vlan, skb->len + ETH_HLEN,
				 err == NET_RX_SUCCESS, true);
				       ...
				       struct macvlan_dev *vlan)
{
	...
	if (skb_vlan_tag_present(nskb)) {
		...
		skb_vlan_push(nskb, nskb->vlan_proto, nskb->vlan_tci);
	}
...
static int __init macvlan_init_module(void)
{
	...
	if (err < 0)
		goto err1;
	pr_info("Macvlan driver %s initialized\n", ATAYA_VER_STR);
	return 0;
err1:
...
static void __exit macvlan_cleanup_module(void)
{
	...
	unregister_netdevice_notifier(&macvlan_notifier_block);
	pr_info("Macvlan driver %s de-initialized\n", ATAYA_VER_STR);
}
